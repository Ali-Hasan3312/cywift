  ...
  return (
    ...
    <div className=" h-12 w-[1100px] flex items-center justify-between mt-2">
      <div className="flex items-center gap-4">
        <div className={`h-9 w-[8rem] rounded-lg cursor-pointer ${location.pathname==='/Risk/Register'? 'bg-black text-white': 'bg-white text-black'} underline flex items-center justify-center`}>
          <Link  className="w-full h-full flex items-center justify-center" to={"/Risk/Register"}>Risk Register</Link>
        </div>
        <div className={`h-9 cursor-pointer w-[11rem] rounded-lg   text-opacity-90 ${location.pathname==='/Risk/Dashboard'? 'bg-black text-white': 'bg-white text-black'} underline flex items-center justify-center`}>
          <Link className="w-full h-full flex items-center justify-center" to={"/Risk/Dashboard"}>Risk Dashboard</Link>
        </div>
        <div className={`h-9 w-[9rem] rounded-lg cursor-pointer  ${location.pathname==='/Risk/Map'? 'bg-black text-white': 'bg-white text-black'}  text-opacity-90 underline flex items-center justify-center`}>
